...
export class SubscriptionController {
  ...
  @ApiOkResponse({
    type: [Subscription],
    description: 'Subscriptions have been successfully received',
  })
  ...
  @ApiOkResponse({
    type: Subscription,
    description: 'Subscription has been successfully received',
  })
  ...
  @ApiOkResponse({
    type: [Subscription],
    description: "User's subscriptions have been successfully received",
  })
  @ApiUnauthorizedResponse({
    description:
      'User is not authorized - JWT token was not correct/was not provided.',
  })
  async getUserSubscriptions(@Request() req): Promise<UserSubscription[]> {
    const userId = req.user.id;

    try {
      return await this.subscriptionService.getBoughtSubscriptionsByUserId(
        userId,
      );
    } catch {
      throw new BadRequestException('Invalid operation!');
  ...
  async getCurrentUserSubscriptions(
    ...
    try {
      return await this.subscriptionService.getCurrentSubscriptionsByUserId(
        userId,
      );
    } catch {
      throw new BadRequestException('Invalid operation!');
  ...
  @ApiCreatedResponse({
    ...
    description: 'Subscription was successfully bought',
  })
  ...
  async buySubscription(
    ...
    try {
      return await this.subscriptionService.processSubscriptionPurchase(
        userId,
        body,
      );
    } catch (err) {
  ...
  @ApiOkResponse({
    description: "User's subscription was successfully canceled",
  })
  @ApiBadRequestResponse({ description: 'Provided request body was wrong' })
  @ApiNotFoundResponse({
    description:
      'User does not have subscription with id, that was provided in request body',
  })
  @ApiUnauthorizedResponse({
    description:
      'User is not authorized - JWT token was not correct/was not provided.',
