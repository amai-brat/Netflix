...
describe('SubscriptionService', () => {
  ...
  const fakeSubscriptions: Subscription[] = [
    ...
    },
    {
      id: 2,
      name: 'Мультфильмы',
      description: 'Мультфильмы подписка',
      max_resolution: 1080,
      price: 322,
    },
  ];
  ...
  const mockUserRepository = {
    findOneByOrFail: jest
      .fn()
      .mockImplementation(async (findOptions: { id: number }) => {
        const user = fakeUsers.find((us) => us.id == findOptions.id);
  ...
  const mockSubscriptionRepository = {
    find: jest
      .fn()
      .mockImplementation(async () => Promise.resolve(fakeSubscriptions)),
    findOneByOrFail: jest
      .fn()
      .mockImplementation(async (findOptions: { id: number }) => {
        const subscription = fakeSubscriptions.find(
          (s) => s.id == findOptions.id,
        );
        if (!subscription) {
  ...
  const mockUserSubscriptionRepository = {
    create: jest.fn().mockImplementation((us) => {
      us.id = 228;
      us.status = 1;
      return us;
    }),
    save: jest
      .fn()
      .mockImplementation(async (userSubscription: UserSubscription) => {
        let savedElementIndex: number;

        const existingElementIndex = fakeUserSubscriptions.findIndex(
          (us) =>
    ...
    remove: jest
      .fn()
      .mockImplementation(async (userSubscription: UserSubscription) => {
        const removingElementIndex =
          fakeUserSubscriptions.indexOf(userSubscription);
        fakeUserSubscriptions.splice(removingElementIndex, 1);
      }),
    findBy: jest
      .fn()
      .mockImplementation(
        async (findOptions: { userId: number; subscriptionId: number }) => {
          let userSubscription;
          if (findOptions.subscriptionId !== undefined) {
            userSubscription = fakeUserSubscriptions.filter(
              (s) =>
                s.userId == findOptions.userId &&
                s.subscriptionId == findOptions.subscriptionId,
            );
          } else {
            userSubscription = fakeUserSubscriptions.filter(
              (s) => s.userId == findOptions.userId,
            );
          }
    ...
    findOneByOrFail: jest
      .fn()
      .mockImplementation(async (findOptions: { id: number }) => {
        const userSubscription = fakeUserSubscriptions.find(
          (us) => us.id === findOptions.id,
        );
        if (!userSubscription) {
  ...
  it('should return subscription with selected id', async () => {
    const randomSubscriptionId =
      fakeSubscriptions[1].id;

    const expectedSubscription = fakeSubscriptions.find(
      (s) => (s.id = randomSubscriptionId),
    );
  ...
  it('should save purchased subscription and return it', async () => {
    ...
    const result = await service.processSubscriptionPurchase(user.id, {
  ...
  it('should return all bought subscriptions by user nickname', async () => {
    ...
    const expectedUserSubscriptions = fakeUserSubscriptions.filter(
      (us) => us.userId == randomUser.id,
    );
  ...
  it('should cancel subscription', async () => {
    ...
    const randomSubscriptionId =
      fakeSubscriptions[Math.floor(Math.random() * fakeSubscriptions.length)]
        .id;
    await service.processSubscriptionPurchase(randomUser.id, {
      subscriptionId: randomSubscriptionId,
      card: null,
    });
